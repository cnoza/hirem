...
reserving_data <- reserving_data %>% 
  mutate(q.size.recov = ifelse(size<0, p.size.recov/(1-p.size.recov), 0))

# not needed but to check
# ultimate <- reserving_data %>% 
#   dplyr::group_by(claim.nr) %>% 
#   dplyr::summarise(total.size.pay = sum(size.pay))
# 
# reserving_data <- merge(reserving_data, ultimate, by="claim.nr", all.x=T)
# 
# reserving_data <- reserving_data %>%
#   mutate(p.size.recov.check = ifelse(size<0, -size/total.size.pay, 0))
