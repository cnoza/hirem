...
hyper_grid_se <- expand.grid(eta = 1, max_depth = 1, min_child_weight = 100, subsample = c(.7,.75,.8,.85,.9,.95,1), colsample_bynode = 1, gamma = 0, lambda = 1, alpha = 0)
hyper_grid_pa <- expand.grid(eta = .5, max_depth = 3, min_child_weight = 100, subsample = c(.7,.75,.8,.85,.9,.95,1), colsample_bynode = 1, gamma = 0, lambda = 1, alpha = 0)
hyper_grid_si <- expand.grid(eta = .05, max_depth = 1, min_child_weight = 100, subsample = c(.7,.75,.8), colsample_bynode = 1, gamma = 0, lambda = 1, alpha = 0)
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('settlement', objective = 'binary:logistic',
            ...
            nfolds = 5, gridsearch_cv = T, hyper_grid = hyper_grid_se, 
            verbose = F) %>%
  layer_xgb('payment', objective = 'binary:logistic',
            eval_metric = 'logloss', grow_policy = 'lossguide', 
            nfolds = 5, gridsearch_cv = T, hyper_grid = hyper_grid_pa, 
            verbose = F) %>%
  layer_xgb('size', objective = 'reg:gamma', 
            eval_metric = 'gamma-deviance', grow_policy = 'lossguide', 
            nfolds = 5, gridsearch_cv = T, hyper_grid = hyper_grid_si, 
            early_stopping_rounds = 10, nrounds = 725,
            verbose = F, 
            filter = function(data){data$payment == 1})
```
...
bounds <- list(
  dnn_hidden_1 = c(2L,7L),
  dnn_hidden_2 = c(2L,7L),
  dnn_hidden_3 = c(2L,7L)
)
...
model_dnn <-  hirem(reserving_data) %>%
  ...
  layer_dnn('settlement', distribution = 'bernoulli',
            ...
            bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 1, bayesOpt_bounds = bounds, bayesOpt_step = 10,
...
bounds <- list(
  dnn_hidden_1 = c(2L,6L),
  dnn_hidden_2 = c(2L,6L),
  dnn_hidden_3 = c(2L,6L)
)
