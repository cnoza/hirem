...
years <- 24               # number of occurrence years
...
observed_data   <- reserving_data %>% filter(calendar.year <= 11)
prediction_data <- reserving_data %>% filter(calendar.year > 11)
```
...
names(weight) <- paste0('dev.year',1:11)
weight
```
...
model_glm  <- hirem(reserving_data) %>%
  split_data(observed = function(df) df %>% filter(calendar.year <= 11, open == 1)) %>%
...
```{r}
# Covariates
covariates_gbm <- c('type', 'dev.year.fact', 'rep.month', 'rep.year.fact', 'calendar.year')
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('settlement', objective = 'binary:logistic',
            ...
            #gridsearch_cv=T, hyper_grid = hyper_grid,
...
# Bounds specification for Bayesian optimization
bounds <- list(
  dnn_hidden_1 = c(20L,60L),
  dnn_hidden_2 = c(20L,60L),
  dnn_hidden_3 = c(20L,60L)
  #batch_size = c(1L,10L)
)

hyper_grid <- expand.grid(
  dnn_hidden_1 = seq(from=0,to=50,by=10),
  dnn_hidden_2 = seq(from=0,to=50,by=10),
  dnn_hidden_3 = seq(from=0,to=50,by=10)
)
...
model_dnn <-  hirem(reserving_data) %>%
  ...
  layer_dnn('settlement', distribution = 'bernoulli',
            bayesOpt = F, bayesOpt_min = T, bayesOpt_iters_n = 3, bayesOpt_bounds = bounds,
            gridsearch_cv=F, hyper_grid=hyper_grid,
            use_embedding = F, #output_dim = 1, embedding_var = c('rep.month', 'rep.year.fact'),
...
The bias regularization being activated, the neural network architecture can be recovered as follows:
```{r}
# Settlement
model_dnn$layers$settlement$fit.biased
```

```{r}
# Payment
model_dnn$layers$payment$fit.biased
```

```{r}
# Size
model_dnn$layers$size$fit.biased
```
...
model_cann <-  hirem(reserving_data) %>%
  ...
  layer_cann('settlement', distribution = 'bernoulli',
               formula.glm = formula_settle_glm, bias_regularization = T,
               bayesOpt = F, bayesOpt_min = T, bayesOpt_iters_n = 3, bayesOpt_bounds = bounds,
               gridsearch_cv=T, hyper_grid=hyper_grid, random_trials = 50,
               use_embedding = F, #embedding_var = c('rep.month', 'rep.year.fact'),
  ...
  layer_cann('payment', distribution = 'bernoulli',
               formula.glm = formula_pay_glm, bias_regularization = T,
               bayesOpt = F, bayesOpt_min = T, bayesOpt_iters_n = 3, bayesOpt_bounds = bounds,
               gridsearch_cv=T, hyper_grid=hyper_grid, random_trials = 50,
               use_embedding = F, #embedding_var = c('rep.month', 'rep.year.fact'),
               family_for_glm = binomial(),
  ...
  layer_cann('size', distribution = 'gamma',
               formula.glm = formula_size_glm, bias_regularization = T,
               bayesOpt = F, bayesOpt_min = T, bayesOpt_iters_n = 3, bayesOpt_bounds = bounds,
               gridsearch_cv=T, hyper_grid=hyper_grid, random_trials = 50,
               use_embedding = F, #embedding_var = c('rep.month', 'rep.year.fact'),
...
```{r}
# Settlement
model_cann$layers$settlement$fit.biased
```

```{r}
# Payment
model_cann$layers$payment$fit.biased
```

```{r}
# Size
model_cann$layers$size$fit.biased
```
...
simul_glm <- simulate(model_glm,
                      ...
                      data = model_glm$data_observed %>% dplyr::filter(calendar.year == 11),
                      balance.correction = TRUE)

simul_gbm <- simulate(model_gbm,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= 10, settlement == 0)},
                      data = model_gbm$data_observed %>% dplyr::filter(calendar.year == 11),
                      balance.correction = TRUE)

simul_xgb <- simulate(model_xgb,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= 10, settlement == 0)},
                      data = model_xgb$data_observed %>% dplyr::filter(calendar.year == 11),
                      balance.correction = TRUE)

simul_dnn <- simulate(model_dnn,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= 10, settlement == 0)},
                      data = model_dnn$data_observed %>% dplyr::filter(calendar.year == 11),
                      balance.correction = TRUE)

simul_cann <- simulate(model_cann,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= 10, settlement == 0)},
                      data = model_cann$data_observed %>% dplyr::filter(calendar.year == 11),
                      balance.correction = TRUE)
...
# Predictions
obs_open_total <- prediction_data %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)) %>% pull(Total)
cl_open_total  <- sum(cl_open)
glm_open_total <- simul_glm %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
gbm_open_total <- simul_gbm %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
xgb_open_total <- simul_xgb %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
dnn_open_total <- simul_dnn %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
cann_open_total <- simul_cann %>% filter(calendar.year != 12) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
