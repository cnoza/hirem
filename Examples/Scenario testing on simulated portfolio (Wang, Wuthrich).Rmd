...
time_unit <- 1          # we consider yearly claims development
...
claims <- claims %>% filter(Status != 'IBNR')
...
reserving_data <- merge(paid, claims, by="Id", all.x = T, all.y = T)

# Case: time_unit = 1 (yearly development periods)
reserving_data <- reserving_data %>%
  mutate(calendar.year = EventMonth,
         rep.year = RepMonth,
         dev.year = calendar.year - rep.year + 1,
         settlement.year = SetMonth,
         payment = as.numeric(Paid>0),
         settlement = as.numeric(dev.year == settlement.year),
         open = 1,
         size = Paid,
         type = Type,
         rep.month = month(ymd(RepDate))) %>%
  arrange(Id, EventId) %>%
  select(-EventMonth, -RepMonth, -SetMonth, -Paid, -Type)

reserving_data$dev.year.fact <- factor(reserving_data$dev.year, levels = 1:years)
reserving_data$rep.year.fact <- factor(reserving_data$rep.year, levels = 1:years)
...
reserving_data <- reserving_data %>% filter(size > 0)
...
update <- function(data) {
  ...
  data$dev.year.fact <- factor(data$dev.year, levels = 1:years)
