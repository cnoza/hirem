...
## Introduction
This document implements hierarchical reserving models based on the extended hirem package. The input data is generated using the simulation engine from Wang and Wuthric:
...
reserving_data <- merge(paid, claims, by="Id")
...
reserving_data <- reserving_data %>%
  mutate(calendar.year = floor(EventMonth/12)+1,
         ...
         size = Paid) %>%
  arrange(Id, EventId) %>%
  select(-EventMonth, -RepMonth, -OpenInd, -PayInd, -SetMonth, -Paid)

# Case: time_unit = 1/12 (monthly development periods)
# reserving_data <- reserving_data %>% 
#   # We discard IBNR claims
#   filter(Status != 'IBNR') %>%
#   mutate(calender.year = floor(EventMonth/12)+1,
#          rep.year = floor(RepMonth/12)+1,
#          dev.year = floor((EventMonth-RepMonth)/12)+1,
#          settlement.year = floor(SetMonth/12)+1,
#          payment = PayInd,
#          settlement = as.numeric(dev.year == settlement.year)) %>%
#   arrange(Id, EventId)

reserving_data$dev.year.fact <- factor(reserving_data$dev.year, levels = 1:17)
