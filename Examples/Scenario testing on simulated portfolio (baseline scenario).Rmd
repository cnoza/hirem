...
One can retrieve the best hyperparameters found by the Bayesian optimization as follows:
```{r}
# For settlement
optObj <- model_xgb$layers$settlement$optObj
getBestPars(optObj)

# For payment
optObj <- model_xgb$layers$payment$optObj
getBestPars(optObj)

# For size
optObj <- model_xgb$layers$size$optObj
getBestPars(optObj)
```

The optimal number of rounds can be retrieved as follows:
```{r}
...
To obtain all hyperparameters used for training, one can procede as follows:
```{r}
# For settlement (for example)
print(model_xgb$layers$settlement$best_params)
```
...
# Bounds specification for Bayesian optimization
bounds <- list(
  mlp_hidden_1 = c(20L,60L),
  mlp_hidden_2 = c(20L,60L),
  mlp_hidden_3 = c(20L,60L)
)
...
```
We plot the embedding layers trained for response variable `settlement':
```{r}
emb_dev.year.fact <- (model_dnn$layers$settlement$fit.biased$get_layer("el_dev.year.fact") %>% get_weights())[[1]] %>%
  as.data.frame() %>%
  add_column(levels(factor(observed_data$dev.year.fact))) %>%
  rlang::set_names(c("Dim_1", "Dim_2", "Label"))

ggplot(emb_dev.year.fact, aes(x = Dim_1, y = Dim_2 ,label = Label)) + 
  geom_point(size = 1) + 
  geom_text(aes(label = Label), hjust = 0, vjust = 0, size = 3) + 
  ggtitle("2-dim embedding: dev.year.fact")

emb_type <- (model_dnn$layers$settlement$fit.biased$get_layer("el_type") %>% get_weights())[[1]] %>%
  as.data.frame() %>%
  add_column(levels(factor(observed_data$type))) %>%
  rlang::set_names(c("Dim_1", "Dim_2", "Label"))

ggplot(emb_type, aes(x = Dim_1, y = Dim_2 ,label = Label)) + 
  geom_point(size = 1) + 
  geom_text(aes(label = Label), hjust = 0, vjust = 0, size = 3) + 
  ggtitle("2-dim embedding: type")
```
...
model_cann <-  hirem(reserving_data) %>%
  ...
  layer_cann('settlement', distribution = 'bernoulli',
               ...
               bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 1, bayesOpt_bounds = bounds,
               use_embedding = T,
               family_for_glm = binomial(link=cloglog),
               step_normalize = T,
               loss = 'binary_crossentropy',
               metrics = 'binary_crossentropy',
               optimizer = optimizer_nadam(),
...
```{r}
# Settlement
model_cann$layers$settlement$fit
```
...
```
We plot the embedding layer trained for response variable `settlement':
```{r}
emb_dev.year.fact <- (model_cann$layers$settlement$fit.biased$get_layer("el_dev.year.fact") %>% get_weights())[[1]] %>%
  as.data.frame() %>%
  add_column(levels(factor(observed_data$dev.year.fact))) %>%
  rlang::set_names(c("Dim_1", "Label"))

ggplot(emb_dev.year.fact, aes(x = Dim_1, y = Label ,label = Label)) + 
  geom_point(size = 1) + 
  geom_text(aes(label = Label), hjust = 0, vjust = 0, size = 3) + 
  ggtitle("1-dim embedding: dev.year.fact")
```


```{r}
# Size
model_cann$layers$size$fit
```
...
mlp_open_total <- simul_dnn %>% filter(calendar.year != 10) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
...
mlp_pay_total <- simul_dnn %>% summarise(Total = sum(payment)/nsim) %>% pull(Total)
...
mlp_size_total <- simul_dnn %>% summarise(Total = sum(size)/nsim) %>% pull(Total)
