...
# Bounds specification for Bayesian optimization
bounds <- list(
  mlp_hidden_1 = c(20L,50L),
  mlp_hidden_2 = c(20L,50L),
  mlp_hidden_3 = c(20L,50L)
)
...
model_mlp <-  hirem(reserving_data) %>%
  ...
  layer_mlp_keras('settlement', distribution = 'bernoulli',
                  bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 5, bayesOpt_bounds = bounds,
                  step_log = F,
                  step_normalize = F,
  ...
  layer_mlp_keras('size', distribution = 'gamma',
                  bayesOpt = F, bayesOpt_min = T, bayesOpt_iters_n = 5, bayesOpt_bounds = bounds,
                  step_log = F,
                  step_normalize = F,
...
```{r}
# Settlement
model_mlp$layers$settlement$fit.biased
```
```{r}
# Payment
model_mlp$layers$payment$fit.biased
```

```{r}
# Size
model_mlp$layers$size$fit.biased
```
...
model_cann <-  hirem(reserving_data) %>%
  ...
  layer_cann('settlement', distribution = 'bernoulli',
               formula.glm = formula_settle_glm, bias_regularization = FALSE,
               bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 5, bayesOpt_bounds = bounds,
  ...
  layer_cann('payment', distribution = 'bernoulli',
               formula.glm = formula_pay_glm, bias_regularization = FALSE,
               bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 5, bayesOpt_bounds = bounds,
  ...
  layer_cann('size', distribution = 'gamma',
               formula.glm = formula_size_glm, bias_regularization = FALSE,
               bayesOpt = T, bayesOpt_min = T, bayesOpt_iters_n = 5, bayesOpt_bounds = bounds,
...
mlp_pay_total <- simul_mlp %>% summarise(Total = sum(payment)/nsim) %>% pull(Total)
...
mlp_size_total <- simul_mlp %>% summarise(Total = sum(size)/nsim) %>% pull(Total)
