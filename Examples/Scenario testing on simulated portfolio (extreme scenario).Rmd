...
## Introduction
This version of this document extends a previous one written by Jens Robben, in which hierarchical GLM and hierarchical GBM are compared. In the present version, hierarchical XGB, hierarchical MLP and hierarchical CANN are further considered. For the latter ones, however, no proper tuning strategy is (yet) followed to justify the chosen model hyperparameters, which may reflect in the results presented below.
...
mlp_pay_total <- simul_dnn %>% filter(calendar.year != 10) %>% summarise(Total = sum(payment)/nsim) %>% pull(Total)
...
mlp_size_total <- simul_dnn %>% filter(calendar.year != 10) %>% summarise(Total = sum(size)/nsim) %>% pull(Total)
