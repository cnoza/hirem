...
We illustrate the use of a CANN model as layer in a hierarchical model. The `keras` package is used to fit the CANN model. For more information on such models, see Schelldorfer, Jürg and Wuthrich, Mario V., Nesting Classical Actuarial Models into Neural Networks (January 22, 2019). Available at SSRN: https://ssrn.com/abstract=3320525 or http://dx.doi.org/10.2139/ssrn.3320525. 
...
For what follows, we use the following model specification as benchmark.
...
We then compute the simulated RBNS reserves and compare them with the true value. Note that a bias regularization on the fitted CANN is implemented in `layer_cann` to correct for the bias (when it makes sense). For more information, see Ferrario, Andrea and Noll, Alexander and Wuthrich, Mario V., Insights from Inside Neural Networks (April 23, 2020), p.52.
