...
model_glm  <- hirem(reserving_data) %>%
  ...
  layer_glm(name = 'size.recov', 'family' = Gamma(link = 'log'), 
...
family.size.recov <- model_glm$layers$size.recov$method_options
res.tab.size.recov <- covariate_selection('size.recov', all_covariates, family.size.recov, d_size_recov, weight, minimize='deviance')
...
model_glm <- hirem::fit(model_glm,
                 ...
                 size.recov = formula_size_recov_glm)
```
...
model_gbm <- hirem(reserving_data) %>%
  ...
  layer_gbm('size.recov', distribution = 'gamma', bag.fraction = 0.75, n.minobsinnode = 100,
...
model_gbm <- hirem::fit(model_gbm,
                 ...
                 size.recov = paste0('size.recov ~ 1 + ', paste0(c(covariates_gbm,'settlement'), collapse = ' + ')))
```
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('size.pay', objective = 'reg:gamma', 
            ...
            filter = function(data){data$payment == 1}) %>%
  layer_xgb('size.recov', objective = 'reg:gamma', 
            eval_metric = 'gamma-deviance', grow_policy = 'lossguide',
...
model_xgb <- hirem::fit(model_xgb,
                        ...
                        size.recov = paste0('size.recov ~ ', paste0(c(covariates_xgb,'settlement'), collapse = ' + ')))
```
...
model_dnn <-  hirem(reserving_data) %>%
  ...
  layer_dnn('size.pay', distribution = 'gamma',
            ...
            patience = 20,
            filter = function(data){data$payment == 1}) %>%
  layer_dnn('size.recov', distribution = 'gamma',
            bias_regularization = T, 
            hidden = c(10,10,20),
            loss = gamma_deviance_keras,
            metrics = metric_gamma_deviance_keras,
...
model_dnn <- hirem::fit(model_dnn,
                        ...
                        size.recov = paste0('size.recov ~ ', paste0(c(covariates_dnn,'settlement'), collapse = ' + ')))
```
...
# Print Results
c('Actual' = obs_recov_total, 'Hierarchical GLM' = glm_recov_total, 'Hierarchical GBM' = gbm_recov_total, 'Hierarchical XGB' = xgb_recov_total, 'Hierarchical DNN' = dnn_recov_total, 'Hierarchical CANN' = cann_recov_total)
```
...
# Print Results
c('Actual' = obs_size_pay_total, 'Chain-Ladder' = cl_size_pay_total, 'Hierarchical GLM' = glm_size_pay_total, 'Hierarchical GBM' = gbm_size_pay_total, 'Hierarchical XGB' = xgb_size_pay_total, 'Hierarchical DNN' = dnn_size_pay_total, 'Hierarchical CANN' = cann_size_pay_total)
```
...
# Print Results
c('Actual' = obs_size_recov_total, 'Chain-Ladder' = cl_size_recov_total, 'Hierarchical GLM' = glm_size_recov_total, 'Hierarchical GBM' = gbm_size_recov_total, 'Hierarchical XGB' = xgb_size_recov_total, 'Hierarchical DNN' = dnn_size_recov_total, 'Hierarchical CANN' = cann_size_recov_total)
```
