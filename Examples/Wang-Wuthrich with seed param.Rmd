...
covariate_selection <- function(response_variable, all_covariates, family, data, weight, minimize="deviance") {
  ...
  while(length(all_covariates) > 0) {
    ...
    for(covariate in all_covariates) {
      ...
      step = append(step,i)
      if(m_glm$deviance < deviance_sav) {
...
d <- reserving_data %>% filter(calendar.year <= 10, open == 1)
all_covariates <- c('type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age')
res.tab.settle <- covariate_selection('settlement',all_covariates, binomial(link = logit), d, weight)
print(res.tab.settle)

# Covariate selection for payment layer
all_covariates <- c('settlement', 'type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age')
res.tab.pay <- covariate_selection('payment',all_covariates, binomial(link = logit), d, weight)
print(res.tab.pay)

# Covariate selection for size layer
all_covariates <- c('settlement', 'type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age')
d_size <- reserving_data %>% filter(calendar.year <= 10, open == 1, payment == 1)
res.tab.size <- covariate_selection('size',all_covariates, Gamma(link = 'log'), d_size, weight)
print(res.tab.size)
