...
reserving_data <- simulate_scenario_baseline(seed = seed, n = 125000, gen.recoveries = TRUE)
...
xgb_param_size_recov   <- list('eta' = .05, 'max_depth' = 3, 'nrounds' = 200, 'subsample' = 1)
...
```{r message=FALSE, warning=FALSE}
# Covariates
covariates_dnn <- c('settlement', 'type', 'dev.year.fact', 'rep.month', 'devYearMonth')
...
simul_dnn2 <- simulate(model_dnn2,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= 9, settlement == 0)},
                      data = model_dnn2$data_observed %>% dplyr::filter(calendar.year == 9),
                      balance.correction = TRUE)
...
dnn2_open_total <- simul_dnn2 %>% filter(calendar.year != 10) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
...
c('Actual' =  obs_open_total, 
  ...
  'Hierarchical XGB' = xgb_open_total, 
  'Hierarchical DNN/GLM' = dnn_open_total, 
  'Hierarchical DNN/GBM' = dnn2_open_total, 
...
c('Actual' = obs_pay_total, 
  ...
  'Hierarchical XGB' = xgb_pay_total, 
  'Hierarchical DNN/GLM' = dnn_pay_total, 
  'Hierarchical DNN/GBM' = dnn2_pay_total, 
...
c('Actual' = obs_recov_total, 
  ...
  'Hierarchical XGB' = xgb_recov_total, 
  'Hierarchical DNN/GLM' = dnn_recov_total, 
  'Hierarchical DNN/GBM' = dnn2_recov_total, 
...
c('Actual' = obs_size_pay_total, 
  ...
  'Hierarchical XGB' = xgb_size_pay_total, 
  'Hierarchical DNN/GLM' = dnn_size_pay_total, 
  'Hierarchical DNN/GBM' = dnn2_size_pay_total, 
...
update_simul <- function(simul) {
  ...
  simul <- simul %>% 
    dplyr::select(-total.size, -size.recov) %>%
    dplyr::group_by(simulation, claim.nr) %>%
    dplyr::mutate(total.size = sum(size.pay)) %>%
    dplyr::ungroup() %>%
    mutate(p.size.recov = q.size.recov / (1+q.size.recov), 
           size.recov = p.size.recov * total.size)
...
c('Actual' = obs_size_recov_total, 
  ...
  'Hierarchical XGB' = xgb_size_recov_total, 
  'Hierarchical DNN/GLM' = dnn_size_recov_total, 
  'Hierarchical DNN/GBM' = dnn2_size_recov_total, 
...
c('Actual' = obs_size_total, 
  ...
  'Hierarchical XGB' = xgb_size_pay_total - xgb_size_recov_total, 
  'Hierarchical DNN/GLM' = dnn_size_pay_total - dnn_size_recov_total, 
  'Hierarchical DNN/GBM' = dnn2_size_pay_total - dnn2_size_recov_total, 
