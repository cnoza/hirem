...
## Introduction
This document implements hierarchical reserving models based on the extended hirem package. The input data is generated using the baseline scenario and includes recoveries.
...
formula_recov_glm    <- "recovery ~ settlement + devYearMonth + rep.month + type"
#formula_size_pay_glm   <- res.tab.size.pay$m_formula[[1]]
formula_size_pay_glm   <- "size.pay ~ devYearMonth + type + settlement + rep.month"
#formula_size_recov_glm    <- res.tab.size.recov$m_formula[[1]]
formula_size_recov_glm    <- "q.size.recov ~ devYearMonth + settlement + type + rep.month"
```
...
gbm_param_settle <- list('n.trees' = 25, 'interaction.depth' = 6, 'shrinkage' = 0.2)
...
triangle_recovery <- construct_triangle(data = observed_data, group.var1 = 'rep.year',
                                       group.var2 = 'dev.year', value = 'recovery', cumulative = FALSE)
...
cl_recov  <- chainLadder(triangle_recovery, is_cumulatif = FALSE)
...
cl_recov_total  <- sum(cl_recov, na.rm=T)
