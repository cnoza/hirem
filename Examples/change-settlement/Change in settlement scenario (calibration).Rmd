...
The seed number is:
```{r}
# We set the seed number to 1 for calibration purposes
seed = 1
cat(sprintf("%s", seed))
```
...
xgb_param_size   <- list('eta' = .2, 'max_depth' = 1, 'nrounds' = 311, 'subsample' = .75)
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('size', objective = 'reg:gamma', 
            ...
            gridsearch_cv = TRUE, hyper_grid = hyper_grid, subsample = .75,
            filter = function(data){data$payment == 1})
```
