...
# For the calibration, we use seed=1
seed=1

# The complete portfolio
reserving_data <- simulate_scenario_claim_mix(seed = seed, n= 125000)
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('settlement', objective = 'binary:logistic', eval_metric = 'logloss', grow_policy = 'lossguide',
            gridsearch_cv = TRUE, hyper_grid = hyper_grid2, nrounds=10) %>%
  ...
  layer_xgb('size', objective = 'reg:gamma', 
            ...
            gridsearch_cv = TRUE, hyper_grid = hyper_grid, nrounds=1500,
            filter = function(data){data$payment == 1})
```
...
hyper_grid <- expand.grid(
  dnn_hidden_1 = seq(from=0,to=40,by=10),
  dnn_hidden_2 = seq(from=0,to=40,by=10),
  dnn_hidden_3 = seq(from=0,to=40,by=10) 
)
