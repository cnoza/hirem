...
years <- 30               # number of occurrence years
I <- years/time_unit      # number of development periods

source("~/R-projects/hirem/Examples/wang-wuthrich/Tools/functions simulation1.R")
...
reserving_data <- d4 %>% 
  mutate(rep.year = floor(RepMonth/12)+1-2,
         calendar.year = dev.year + rep.year - 1,
         settlement.year = floor(SetMonth/12)+1-2,
         payment = as.integer(size>0),
         recovery = as.integer(size<0),
         size.pay = ifelse(size>0,size,0),
         size.recov = ifelse(size<0, -size, 0),
         occ.month = AccMonth,
         occ.month12 = factor(ifelse(occ.month %% 12 == 0, 12, occ.month %% 12)),
         occ.date = AccDate,
         occ.weekday = factor(AccWeekday),
         rep.date = RepDate,
         age = Age,
         age.fact = factor(Age),
         age.cat = cut(Age, breaks=unique(quantile(Age,probs=seq.int(0,1, by=1/10))), include.lowest=TRUE),
         RepDelDays.cat = cut(RepDelDays, breaks=unique(quantile(RepDelDays,probs=seq.int(0,1, by=1/10))), include.lowest=TRUE),
...
reserving_data <- reserving_data %>% filter(rep.year >= 1 & rep.year <= year.max & dev.year <= year.max)

reserving_data <- reserving_data %>% 
  dplyr::group_by(Id) %>% 
  dplyr::mutate(total.size.pay = sum(size.pay)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(p.size.recov = ifelse(size<0, -size/total.size.pay, 0),
                q.size.recov = ifelse(size<0, log(p.size.recov/(1-p.size.recov)), 0))

#reserving_data <- reserving_data %>% filter(total.size.pay < 20000)
...
```{r}
reserving_data %>% group_by(age.cat) %>% summarize(n_pay = sum(payment), 
                                                   n_settl = sum(settlement), 
                                                   n_recov = sum(recovery),
                                                   tot_size.pay = sum(size.pay),
                                                   tot_size.recov = sum(size.recov))
```
 
```{r}
reserving_data %>% group_by(dev.year) %>% summarize(n_pay = sum(payment), 
                                                   n_settl = sum(settlement), 
                                                   n_recov = sum(recovery),
                                                   tot_size.pay = sum(size.pay),
                                                   tot_size.recov = sum(size.recov))
...
```{r}
reserving_data %>% group_by(dev.year, rep.year) %>% summarize(n_pay = sum(payment)) %>% filter(dev.year==9)
```

```{r}
reserving_data %>% filter(payment==1) %>% group_by(calendar.year) %>% summarize(n_pay = sum(payment), mean_size.pay = mean(size.pay))
```
 
```{r}
reserving_data %>% group_by(RepDelDays.cat) %>% summarize(n_pay = sum(payment), 
                                                   n_settl = sum(settlement), 
                                                   n_recov = sum(recovery),
                                                   tot_size.pay = sum(size.pay),
                                                   tot_size.recov = sum(size.recov))
...
observed_data   <- reserving_data %>% filter(calendar.year <= year.max)
prediction_data <- reserving_data %>% filter(calendar.year > year.max)
```
...
names(weight) <- paste0('dev.year',1:year.max)
weight
```
...
model_glm  <- hirem(reserving_data) %>%
  split_data(observed = function(df) df %>% filter(calendar.year <= year.max, open == 1)) %>%
...
# all_covariates <- c('type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays.cat', 'occ.weekday')
...
# all_covariates <- c('settlement', 'type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays.cat', 'occ.weekday')
...
formula_settle_glm <- "settlement ~ type + devYearMonth + age.cat + RepDelDays.cat + rep.month + occ.month12 + occ.weekday + calendar.year"
formula_pay_glm <- "payment ~ settlement + devYearMonth + type + age.cat + rep.month + RepDelDays.cat + occ.month12 + occ.weekday + calendar.year"
formula_recov_glm <- "recovery ~ settlement + devYearMonth + type + occ.month12 + rep.month + age.cat + RepDelDays.cat + occ.weekday + calendar.year"
formula_size_pay_glm <- "size.pay ~ type + devYearMonth + settlement + age.cat + occ.month12 + RepDelDays.cat + rep.month + occ.weekday + calendar.year"
formula_size_recov_glm <- "q.size.recov ~ type + devYearMonth + RepDelDays.cat + age.cat + rep.month + occ.weekday + occ.month12 + settlement + calendar.year"
...
gbm_param_settle <- list('n.trees' = 225, 'interaction.depth' = 3, 'shrinkage' = 0.05)
gbm_param_pay    <- list('n.trees' = 225, 'interaction.depth' = 3, 'shrinkage' = 0.05)
gbm_param_recov    <- list('n.trees' = 225, 'interaction.depth' = 3, 'shrinkage' = 0.05)
gbm_param_size_pay   <- list('n.trees' = 700, 'interaction.depth' = 3, 'shrinkage' = 0.05)
gbm_param_size_recov   <- list('n.trees' = 700, 'interaction.depth' = 3, 'shrinkage' = 0.05)
...
```{r}
# Covariates
covariates_gbm <- c('type', 'dev.year.fact', 'rep.month', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays.cat', 'occ.weekday')
...
xgb_param_size   <- list('eta' = .5, 'max_depth' = 3, 'nrounds' = 40, 'subsample' = .75)
...
model_xgb <- hirem(reserving_data) %>%
  ...
  layer_xgb('size.pay', objective = 'reg:gamma', 
            #gridsearch_cv = T, hyper_grid = hyper_grid, nfolds = 5,
            eval_metric = 'gamma-deviance', grow_policy = 'lossguide',
            eta = xgb_param_size$eta, max_depth = xgb_param_size$max_depth, nrounds = xgb_param_size$nrounds, 
            subsample = xgb_param_size$subsample,
            filter = function(data){data$payment == 1}) %>%
  layer_xgb('q.size.recov', objective = 'reg:squarederror', eval_metric = 'rmse',
...
# Covariates
covariates_xgb <- c('type', 'dev.year.fact', 'rep.month', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays', 'occ.weekday')
...
```{r}
#Covariates
covariates_cann <- c('type', 'dev.year.fact', 'rep.month', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays.cat', 'occ.weekday')
...
covariates_dnn <- c('type', 'dev.year.fact', 'rep.month', 'age.cat', 'calendar.year', 'occ.month12', 'RepDelDays', 'occ.weekday')
...
update <- function(data) {
  ...
  data$dev.year.fact <- factor(data$dev.year, levels = 1:year.max)
...
simul_glm <- simulate(model_glm,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= year.max, settlement == 0)},
                      data = model_glm$data_observed %>% dplyr::filter(calendar.year == year.max),
                      balance.correction = TRUE)

simul_gbm <- simulate(model_gbm,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= year.max, settlement == 0)},
                      data = model_gbm$data_observed %>% dplyr::filter(calendar.year == year.max),
                      balance.correction = TRUE)

simul_xgb <- simulate(model_xgb,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= year.max, settlement == 0)},
                      data = model_xgb$data_observed %>% dplyr::filter(calendar.year == year.max),
                      balance.correction = TRUE)

simul_cann <- simulate(model_cann,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= year.max, settlement == 0)},
                      data = model_cann$data_observed %>% dplyr::filter(calendar.year == year.max),
                      balance.correction = TRUE)

simul_dnn <- simulate(model_dnn,
                      nsim = nsim,
                      filter = function(data){dplyr::filter(data, dev.year <= year.max, settlement == 0)},
                      data = model_dnn$data_observed %>% dplyr::filter(calendar.year == year.max),
                      balance.correction = TRUE)
...
settle.evalyear <- observed_data %>% 
  filter(open == 1, calendar.year == year.max) %>%
...
triangle_open[row(triangle_open) + col(triangle_open) == (year.max + 2)] <- 
  (triangle_open[row(triangle_open) + col(triangle_open) == (year.max + 1)] - rev(settle.evalyear$settlement))[1:(year.max-1)]
```
...
```{r}
# Predictions
obs_open_total <- prediction_data %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)) %>% pull(Total)
cl_open_total  <- sum(cl_open)
glm_open_total <- simul_glm %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
gbm_open_total <- simul_gbm %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
xgb_open_total <- simul_xgb %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
cann_open_total <- simul_cann %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)
dnn_open_total <- simul_dnn %>% filter(calendar.year != (year.max+1)) %>% summarise(Total = sum(open)/nsim) %>% pull(Total)

# Print Results
c('Actual' =  obs_open_total, 'Chain-Ladder' = cl_open_total, 'Hierarchical GLM' = glm_open_total, 'Hierarchical GBM' = gbm_open_total, 'Hierarchical XGB' = xgb_open_total, 'Hierarchical CANN/GLM' = cann_open_total, 'Hierarchical DNN/GLM' = dnn_open_total)
```
...
# Print Results
c('Actual' = obs_pay_total, 'Chain-Ladder' = cl_pay_total, 'Hierarchical GLM' = glm_pay_total, 'Hierarchical GBM' = gbm_pay_total, 'Hierarchical XGB' = xgb_pay_total, 'Hierarchical CANN/GLM' = cann_pay_total, 'Hierarchical DNN/GLM' = dnn_pay_total)
```
...
glm_recov_total <- simul_glm %>% summarise(Total = round(sum(recovery)/nsim)) %>% pull(Total)
gbm_recov_total <- simul_gbm %>% summarise(Total = round(sum(recovery)/nsim)) %>% pull(Total)
xgb_recov_total <- simul_xgb %>% summarise(Total = round(sum(recovery)/nsim)) %>% pull(Total)
cann_recov_total <- simul_cann %>% summarise(Total = round(sum(recovery)/nsim)) %>% pull(Total)
dnn_recov_total <- simul_dnn %>% summarise(Total = round(sum(recovery)/nsim)) %>% pull(Total)

# Print Results
c('Actual' = obs_recov_total, 'Hierarchical GLM' = glm_recov_total, 'Hierarchical GBM' = gbm_recov_total, 'Hierarchical XGB ' = xgb_recov_total, 'Hierarchical CANN/GLM' = cann_recov_total, 'Hierarchical DNN/GLM' = dnn_recov_total)
```
...
# Print Results
c('Actual' = obs_size_pay_total, 'Chain-Ladder' = cl_size_pay_total, 'Hierarchical GLM' = glm_size_pay_total, 'Hierarchical GBM' = gbm_size_pay_total, 'Hierarchical XGB' = xgb_size_pay_total, 'Hierarchical CANN/GLM' = cann_size_pay_total, 'Hierarchical DNN/GLM' = dnn_size_pay_total)
```
...
cl_size_recov_total  <- sum(cl_size_recov)
...
# Print Results
c('Actual' = obs_size_recov_total, 'Chain-Ladder' = cl_size_recov_total, 'Hierarchical GLM' = glm_size_recov_total, 'Hierarchical GBM' = gbm_size_recov_total, 'Hierarchical XGB' = xgb_size_recov_total, 'Hierarchical CANN/GLM' = cann_size_recov_total, 'Hierarchical DNN/GLM' = dnn_size_recov_total)
```
