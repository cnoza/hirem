...
reserving_data <- d4 %>% 
  mutate(rep.year = floor(RepMonth/12)+1-2,
         ...
         age = Age,
         age.fact = factor(Age),
         age.cat = cut(Age, breaks=unique(quantile(Age,probs=seq.int(0,1, by=1/5))), include.lowest=TRUE),
...
reserving_data <- reserving_data %>% filter(rep.year >= 1 & rep.year <= 10 & dev.year <= 10)
...
# all_covariates <- c('type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age.cat')
...
# all_covariates <- c('settlement', 'type', 'dev.year.fact', 'rep.month', 'devYearMonth', 'age.cat')
...
formula_settle_glm <- "settlement ~ type + devYearMonth + age.cat + rep.month"
formula_pay_glm <- "payment ~ devYearMonth + settlement + type + age.cat + rep.month"
formula_recov_glm <- "recovery ~ settlement + type + devYearMonth + rep.month + age.cat"
formula_size_pay_glm <- "size.pay ~ type + devYearMonth + settlement + age.cat + rep.month"
formula_size_recov_glm <- "q.size.recov ~ type + devYearMonth + rep.month + settlement + age.cat"
```
...
```{r}
# Covariates
covariates_gbm <- c('type', 'dev.year.fact', 'rep.month', 'age.cat')
...
# Covariates
covariates_xgb <- c('type', 'dev.year.fact', 'rep.month', 'age.cat')
...
```{r}
#Covariates
covariates_cann <- c('type', 'dev.year.fact', 'rep.month', 'age.cat')
...
covariates_dnn <- c('type', 'dev.year.fact', 'rep.month', 'age.cat')
