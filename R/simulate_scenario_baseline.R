#' Simulate portfolios using the simulation engine
#'
#' Simulate portfolios along different scenarios using a simulation engine. Four scenarios are currently provided: a baseline, claim mix, extreme event or change in settlement scenario
#'
#' @param seed Integer. Set the seed for the random number generators.
#' @param n Integer. The number of claims (occurrence dates) you want to generate.
#' @param prob.Type Optional. Numeric vector of size 3 representing the probability distribution of the claim types to be used in the simulated portfolio.
#' @param prob.Hidden Optional. Numeric vector of length 3 representing the probability distribution of the unobserved covariate in the simulated portfolio.
...
#' @param inflation Optional. Numeric vector of size 3 representing the yearly change in the probability distribution of the claim types in the claim mix scneario.
  ...
  if(gen.recoveries) {
    ...
    for(i in 2:9) {
      ...
      if(way==1) {
        ...
        csize <- csize - size*(size_recov > 0) - size_recov
